...
def generate_plan(profile: dict, config: dict, bands: list[dict], power: dict, locked_sessions: list[dict] | None = None) -> dict:
    ...
    start,end=parse(profile["season"]["start_date"]),parse(profile["season"]["end_date"]); commitments,schedule_moves=_recurring_commitments(profile,start,end); modern_schedule=profile.get("recurring_activities") is not None; day=start
    while day<=end:
